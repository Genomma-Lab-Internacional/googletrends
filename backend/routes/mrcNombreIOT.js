    ...
    for(let i=0; i<MrcNombre.MrcNombre0.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre0[i],geo:"MX",hl:"es-419"})
        .then( s => {
          ...
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre0[i]}','${MrcNombreID.MrcNombre0ID[i]}',${r.value[0]})`
    ...
    for(let i=0; i<MrcNombre.MrcNombre1.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre1[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre1[i]}','${MrcNombreID.MrcNombre1ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
    ...
    for(let i=0; i<MrcNombre.MrcNombre2.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre2[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre2[i]}','${MrcNombreID.MrcNombre2ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
    ...
    for(let i=0; i<MrcNombre.MrcNombre3.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre3[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre3[i]}','${MrcNombreID.MrcNombre3ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
    ...
    for(let i=0; i<MrcNombre.MrcNombre4.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre4[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre4[i]}','${MrcNombreID.MrcNombre4ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
    ...
    for(let i=0; i<MrcNombre.MrcNombre5.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre5[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre5[i]}','${MrcNombreID.MrcNombre5ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
    ...
    for(let i=0; i<MrcNombre.MrcNombre6.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre6[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre6[i]}','${MrcNombreID.MrcNombre6ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
    ...
    for(let i=0; i<MrcNombre.MrcNombre7.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre7[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre7[i]}','${MrcNombreID.MrcNombre7ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
    ...
    for(let i=0; i<MrcNombre.MrcNombre8.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre8[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre8[i]}','${MrcNombreID.MrcNombre8ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
    ...
    for(let i=0; i<MrcNombre.MrcNombre9.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre9[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre9[i]}','${MrcNombreID.MrcNombre9ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
    ...
    for(let i=0; i<MrcNombre.MrcNombre10.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre10[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre10[i]}','${MrcNombreID.MrcNombre10ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
    ...
    for(let i=0; i<MrcNombre.MrcNombre11.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre11[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre11[i]}','${MrcNombreID.MrcNombre11ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
    ...
    for(let i=0; i<MrcNombre.MrcNombre12.length; i++) {
      googleTrends.relatedQueries({keyword:MrcNombre.MrcNombre12[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.timelineData
          records.map( r => {
            let insert =  `INSERT INTO IOTMarca (Tiempo,MrcNombre,MrcNombreID,Valor) VALUES ('${r.formattedAxisTime}','${MrcNombre.MrcNombre12[i]}','${MrcNombreID.MrcNombre12ID[i]}',${r.value[0]})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json("BAD",e) )
