    ...
    for(let i=0; i<Competencia.Competencia0.length; i++) {
      googleTrends.relatedQueries({keyword:Competencia.Competencia0[i],geo:"MX",hl:"es-419"})
        .then( s => {
          ...
          records.map( r => {
            let insert =  `INSERT INTO RQCompetencia (BusquedaRelacionada,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.query}','${Competencia.Competencia0[i]}',${r.value},${weekNumber()},${new Date().getFullYear()})`
    ...
    for(let i=0; i<Competencia.Competencia1.length; i++) {
      googleTrends.relatedQueries({keyword:Competencia.Competencia1[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.rankedList[0].rankedKeyword
          records.map( r => {
            let insert =  `INSERT INTO RQCompetencia (BusquedaRelacionada,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.query}','${Competencia.Competencia1[i]}',${r.value},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
    ...
    for(let i=0; i<Competencia.Competencia2.length; i++) {
      googleTrends.relatedQueries({keyword:Competencia.Competencia2[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.rankedList[0].rankedKeyword
          records.map( r => {
            let insert =  `INSERT INTO RQCompetencia (BusquedaRelacionada,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.query}','${Competencia.Competencia2[i]}',${r.value},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
    ...
    for(let i=0; i<Competencia.Competencia3.length; i++) {
      googleTrends.relatedQueries({keyword:Competencia.Competencia3[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.rankedList[0].rankedKeyword
          records.map( r => {
            let insert =  `INSERT INTO RQCompetencia (BusquedaRelacionada,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.query}','${Competencia.Competencia3[i]}',${r.value},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
    ...
    for(let i=0; i<Competencia.Competencia4.length; i++) {
      googleTrends.relatedQueries({keyword:Competencia.Competencia4[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.rankedList[0].rankedKeyword
          records.map( r => {
            let insert =  `INSERT INTO RQCompetencia (BusquedaRelacionada,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.query}','${Competencia.Competencia4[i]}',${r.value},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
    ...
    for(let i=0; i<Competencia.Competencia5.length; i++) {
      googleTrends.relatedQueries({keyword:Competencia.Competencia5[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.rankedList[0].rankedKeyword
          records.map( r => {
            let insert =  `INSERT INTO RQCompetencia (BusquedaRelacionada,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.query}','${Competencia.Competencia5[i]}',${r.value},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
    ...
    for(let i=0; i<Competencia.Competencia6.length; i++) {
      googleTrends.relatedQueries({keyword:Competencia.Competencia6[i],geo:"MX",hl:"es-419"})
        .then( s => {
          let records = JSON.parse(s).default.rankedList[0].rankedKeyword
          records.map( r => {
            let insert =  `INSERT INTO RQCompetencia (BusquedaRelacionada,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.query}','${Competencia.Competencia6[i]}',${r.value},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
