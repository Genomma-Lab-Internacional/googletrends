    ...
    for(let i=0; i<Competencia.Competencia0.length; i++) {
      googleTrends.interestByRegion({keyword:Competencia.Competencia0[i],geo:"MX",hl:"es-419",resolution:"CITY"})
        .then( s => {
          ...
          records.map( r => {
            let insert =  `INSERT INTO InteresPorCiudadCompetencia (Ciudad,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.geoName}','${Competencia.Competencia0[i]}',${r.value[0]},${weekNumber()},${new Date().getFullYear()})`
    ...
    for(let i=0; i<Competencia.Competencia1.length; i++) {
      googleTrends.interestByRegion({keyword:Competencia.Competencia1[i],geo:"MX",hl:"es-419",resolution:"CITY"})
        .then( s => {
          let records = JSON.parse(s).default.geoMapData
          records.map( r => {
            let insert =  `INSERT INTO InteresPorCiudadCompetencia (Ciudad,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.geoName}','${Competencia.Competencia1[i]}',${r.value[0]},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
    ...
    for(let i=0; i<Competencia.Competencia2.length; i++) {
      googleTrends.interestByRegion({keyword:Competencia.Competencia2[i],geo:"MX",hl:"es-419",resolution:"CITY"})
        .then( s => {
          let records = JSON.parse(s).default.geoMapData
          records.map( r => {
            let insert =  `INSERT INTO InteresPorCiudadCompetencia (Ciudad,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.geoName}','${Competencia.Competencia2[i]}',${r.value[0]},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
    ...
    for(let i=0; i<Competencia.Competencia3.length; i++) {
      googleTrends.interestByRegion({keyword:Competencia.Competencia3[i],geo:"MX",hl:"es-419",resolution:"CITY"})
        .then( s => {
          let records = JSON.parse(s).default.geoMapData
          records.map( r => {
            let insert =  `INSERT INTO InteresPorCiudadCompetencia (Ciudad,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.geoName}','${Competencia.Competencia3[i]}',${r.value[0]},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
    ...
    for(let i=0; i<Competencia.Competencia4.length; i++) {
      googleTrends.interestByRegion({keyword:Competencia.Competencia4[i],geo:"MX",hl:"es-419",resolution:"CITY"})
        .then( s => {
          let records = JSON.parse(s).default.geoMapData
          records.map( r => {
            let insert =  `INSERT INTO InteresPorCiudadCompetencia (Ciudad,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.geoName}','${Competencia.Competencia4[i]}',${r.value[0]},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
    ...
    for(let i=0; i<Competencia.Competencia5.length; i++) {
      googleTrends.interestByRegion({keyword:Competencia.Competencia5[i],geo:"MX",hl:"es-419",resolution:"CITY"})
        .then( s => {
          let records = JSON.parse(s).default.geoMapData
          records.map( r => {
            let insert =  `INSERT INTO InteresPorCiudadCompetencia (Ciudad,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.geoName}','${Competencia.Competencia5[i]}',${r.value[0]},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
    ...
    for(let i=0; i<Competencia.Competencia6.length; i++) {
      googleTrends.interestByRegion({keyword:Competencia.Competencia6[i],geo:"MX",hl:"es-419",resolution:"CITY"})
        .then( s => {
          let records = JSON.parse(s).default.geoMapData
          records.map( r => {
            let insert =  `INSERT INTO InteresPorCiudadCompetencia (Ciudad,MrcNombreCompetencia,Valor,Semana,Anio) VALUES ('${r.geoName}','${Competencia.Competencia6[i]}',${r.value[0]},${weekNumber()},${new Date().getFullYear()})`
            connection.query(insert, (error,rows) => error ? console.log(error,"ERROR") : console.log(rows,"DONE"))
            })
          })
        .catch( e => res.status(400).json(e) )
